...
/// 使物体定期循环出现和消失的行为组件
/// </summary>
public class CyclicVisibilityAction : MonoBehaviour, IMechanicAction, ISaveable
{
    [Header("循环设置")]
    ...
    private IEnumerator CycleVisibilityRoutine()
    {
        ...
        cycleStartTime = Time.time; // 记录循环开始的时间
        ...
        while (isActive)
        {
            ...
            isCurrentlyVisible = isVisible; // 更新当前可见状态
    ...
    public string GetUniqueID()
    {
        ...
        {
            uniqueID = $"CyclicVisibility_{gameObject.name}_{GetInstanceID()}";
        }
        return uniqueID;
    }
    ...
    public SaveData Save()
    {
        ...
        data.objectType = "CyclicVisibilityAction";
        
        // 保存当前状态
        data.boolValue = isActive;
        
        // 保存当前可见状态
        data.intValue = isCurrentlyVisible ? 1 : 0;
        
        // 保存循环进度（当前时间与循环开始时间的差值）
        float cycleProgress = 0;
        if (isActive)
        {
            float totalCycleTime = visibleDuration + invisibleDuration;
            float elapsedTime = (Time.time - cycleStartTime) % totalCycleTime;
            cycleProgress = elapsedTime / totalCycleTime;
        }
        data.floatValue = cycleProgress;
    ...
    {
        if (data == null || data.objectType != "CyclicVisibilityAction") return;
        ...
        // 加载可见状态
        isCurrentlyVisible = data.intValue > 0;
        
        // 设置物体可见性
        SetObjectsVisibility(isCurrentlyVisible);
        ...
        if (isActive && !wasActive)
        {
            ...
            // 计算循环开始时间，使其与保存时的进度一致
            float totalCycleTime = visibleDuration + invisibleDuration;
            float cycleProgress = data.floatValue;
            cycleStartTime = Time.time - (cycleProgress * totalCycleTime);
