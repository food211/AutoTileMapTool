...
public class ParameterGroup
{
    ...
    public float phaseVariation;
    public float windStrength;
    public float windFrequency;
    public float elasticAmount;
    ...
    // 组名称，用于在编辑器中显示
    public string groupName = "默认参数组";
    
    /// <summary>
    /// 从预设创建参数组
    /// </summary>
    public static ParameterGroup FromPreset(PlantPersonalityPreset preset)
    {
        var settings = PlantPersonalityPresets.GetPresetSettings(preset);
        var group = new ParameterGroup
        {
            presetType = preset,
            groupName = PlantPersonalityPresets.GetPresetDisplayName(preset) + "参数组"
        };
        
        settings.ExtractToParameterGroup(group);
        return group;
    }
}
...
public class PlantPersonalitySettings
{
    ...
    [Header("=== 整体移动特征 ===")]
    [Range(0f, 1f)]
    [Tooltip("整体移动量 - 植物整体移动的程度")]
    public float wholeObjectMovement = 0.8f;
    
    [Range(0f, 0.2f)]
    [Tooltip("整体旋转量 - 植物整体旋转的程度")]
    public float wholeObjectRotation = 0.05f;
    
    [Header("=== 顶点变形特征 ===")]
    [Range(0.1f, 2.0f)]
    [Tooltip("顶点变形强度 - 顶点变形的强度")]
    public float vertexDeformStrength = 0.8f;
    ...
    /// 提取批处理关键参数到参数组
    /// </summary>
    public void ExtractToParameterGroup(ParameterGroup group)
    {
        group.timeOffset = this.timeOffset;
        group.randomSeed = this.randomSeed;
        group.phaseVariation = this.phaseVariation;
        group.windStrength = this.windStrength;
        group.windFrequency = this.windFrequency;
        group.elasticAmount = this.elasticAmount;
    ...
    /// 从参数组应用批处理关键参数
    /// </summary>
    public void ApplyFromParameterGroup(ParameterGroup group)
    {
        this.timeOffset = group.timeOffset;
        this.randomSeed = group.randomSeed;
        this.phaseVariation = group.phaseVariation;
        this.windStrength = group.windStrength;
        this.windFrequency = group.windFrequency;
        this.elasticAmount = group.elasticAmount;
...
public static class PlantPersonalityPresets
{
    ...
    // 缓存参数组，避免重复创建
    private static Dictionary<PlantPersonalityPreset, ParameterGroup> parameterGroupCache = 
        new Dictionary<PlantPersonalityPreset, ParameterGroup>();
    ...
    /// 获取指定预设的参数组
    /// </summary>
    public static ParameterGroup GetPresetParameterGroup(PlantPersonalityPreset preset)
    {
        // 先检查缓存
        if (parameterGroupCache.ContainsKey(preset))
        {
            return parameterGroupCache[preset];
        }
        
        // 创建新的参数组
        ParameterGroup group = ParameterGroup.FromPreset(preset);
        
        // 缓存参数组
        if (group != null)
        {
            parameterGroupCache[preset] = group;
        }
    ...
    /// 创建标准参数组集合
    /// </summary>
    public static List<ParameterGroup> CreateStandardParameterGroups()
    {
        List<ParameterGroup> groups = new List<ParameterGroup>();
        
        foreach (PlantPersonalityPreset preset in System.Enum.GetValues(typeof(PlantPersonalityPreset)))
        {
            groups.Add(GetPresetParameterGroup(preset));
        }
    ...
    /// 获取植物类型的默认预设
    /// </summary>
    public static PlantPersonalityPreset GetDefaultPresetForPlantType(PlantType plantType)
    {
        switch (plantType)
        {
            case PlantType.Bush:
                return PlantPersonalityPreset.Gentle;
            case PlantType.Grass:
                return PlantPersonalityPreset.Energetic;
            case PlantType.Flower:
                return PlantPersonalityPreset.Delicate;
            case PlantType.Tree:
                return PlantPersonalityPreset.Sturdy;
            case PlantType.Fern:
                return PlantPersonalityPreset.Wild;
            default:
                return PlantPersonalityPreset.Gentle;
        }
    }
