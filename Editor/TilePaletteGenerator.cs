    ...
    private void SliceExportedTexture(string texturePath)
    {
        ...
        private void AutoSliceSpritesSkipEmpty(TextureImporter importer, Texture2D texture, int cellWidth, int cellHeight)
        {
            try
            {
                ...
                Dictionary<string, SpriteRect> existingRectsByName = new Dictionary<string, SpriteRect>();

                // 创建名称到SpriteRect的映射，用于查找现有精灵
                if (existingRects != null && existingRects.Length > 0)
                {
                    foreach (var rect in existingRects)
                    {
                        existingRectsByName[rect.name] = rect;
                    }
                    Debug.Log($"找到 {existingRects.Length} 个现有精灵矩形");
                }
                ...
                for (int row = 0; row < rows; row++)
                {
                    for (int col = 0; col < cols; col++)
                    {
                        ...
                        // 构建精灵名称
                        string spriteName = $"sprite_{col}_{row}";
                        SpriteRect spriteRect;

                        // 检查是否存在同名的现有精灵
                        if (existingRectsByName.TryGetValue(spriteName, out SpriteRect existingRect))
                        {
                            // 复用现有精灵的所有属性，只更新位置和大小
                ...
                Debug.Log($"复用了 {reusedCount} 个现有精灵的设置");
                ...
                Debug.Log($"成功创建 {spriteRects.Count} 个非空白精灵切片，其中复用了 {reusedCount} 个现有设置");
